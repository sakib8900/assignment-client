...
const MyCars = () => {
  ...
  const handleUpdate = (e) => {
    ...
    fetch(`https://assignment-11-server-one-lemon.vercel.app/cars/${selectedCar._id}`, {
      ...
      .then((res) => res.json())
      .then(() => {
        setCars(cars.map((car) => (car._id === selectedCar._id ? { ...car, ...updatedCar } : car)));
        closeModal();
  ...
  return (
    <div className="p-5">
      ...
      {/* Cars Table */}
      {cars.length === 0 ? (
        <p className="text-center text-gray-500">You dos't add any data <Link className="text-red-500 font-bold" to="/addCar">Add Cars</Link></p>
      ) : (
